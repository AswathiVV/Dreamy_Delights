...
def register(req):
    if req.method == 'POST':
        ...
        if not re.match(password_pattern, password):
            messages.error(req, "Password must be at least 6 characters long and include letters and numbers.")
            return redirect(register)
...
def add_category(req):
    if req.method == "POST":
        category_name = req.POST["new_category"].strip()
...
def add_cake(req):
    ...
    return render(req, 'shop/add_cake.html', {"categories": categories})
...
def edit_cake(req,id):
        ...
        if req.method == 'POST':
            ...
            print(file)
            if file:
                Cake.objects.filter(pk=id).update(name=name,price=price,img=file,description=des)   
            else:
                Cake.objects.filter(pk=id).update(name=name,price=price,description=des)   
...
def address_page(req, id):
    ...
    if req.method == 'POST':
        ...
        req.session['address_id'] = user_address.id 
...
def order_payment(req):
    if 'user' in req.session:
        ...
        print(f"DEBUG: order_payment -> Cake: {cake.id}, Quantity: {quantity}") 
...
def pay(req):
    ...
    print(f"DEBUG: pay -> Cake: {cake.id}, Quantity: {quantity}")  
